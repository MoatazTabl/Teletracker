...
class _MessageCardWidgetState extends State<MessageCardWidget> {
  ...
  List<Widget> _buildMessageLines(
    String content,
    ...
    final previousLines =
        previousContent.isNotEmpty ? previousContent.split('\n') : [];
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      lineWidgets.add(
        RichText(
          text: TextSpan(
            children:
                index > 0 && previousLine.isNotEmpty
                ...
                return ExpansionTile(
                  ...
                  children: List.generate(versions.length, (index) {
                    ...
                    final String previousContent =
                        index > 0
                            ? versions[index - 1].data()['message'] ?? ''
                            : '';

                    return ListTile(
                      title: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: _buildMessageLines(
                          content,
                          previousContent,
                          index,
                        ), // إزالة Divider من هنا
                      ),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                                ...
                                children: [
                                  ...
                                  Text(
                                    '${displayTime.hour > 12
                                        ...
                                        : displayTime.hour == 0
                                        ? 12
                                        : displayTime.hour}:${displayTime.minute.toString().padLeft(2, '0')}:${displayTime.second.toString().padLeft(2, '0')} ${displayTime.hour >= 12 ? 'PM' : 'AM'}',
                                    style: Theme.of(
                                      context,
                                    ).textTheme.bodySmall?.copyWith(
                                      fontWeight: FontWeight.bold,
